...
## Основное
- `telega.py` содержит две среды: старая `RobotEnv` (по координатам) и новая `RobotVisionEnv`, которая возвращает двойной стек из верхней и «глазной» камер (84×84, по 4 кадра с каждой) + проприоцепцию (sin/cos yaw). Широкий FOV у верхней камеры снижает риск выпадения цели из поля зрения, а фронтальная камера даёт локальные подсказки.
